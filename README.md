...
In the `src/simulators/` directory, copy `stub.ts` (a simulator that is extremely simple) to whatever file name you want. Then, customize the file. Note that the name property of your class must match the name of the file.
...
Replace anywhere in the file that says `Stub*` or `stub`. For example, replace the import at the top of
