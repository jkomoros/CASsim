...
const extractImportsMap = (definition : TypeDefinition) : ImportsMap => {
	...
	if (definition.type == 'extracted') {
		...
		for (const item of definition.extends) {
			const newMap = {
				[item.import]: [item.value]
			};
...
const extractExtractedTypesMap = (definition : TypeDefinition) : ExtractedTypesMap => {
	...
	if (definition.type == 'extracted') {
		...
		const newExtracedTypeInfo = combineExtractedTypeInfo(subResults[definition.value], subDefinition);
...
export const simulatorTypeFileContents = (simulatorName : string, config : OptionsConfig | OptionsConfigMap, fileName : string) => {
	...
	const extractedTypesContent = Object.entries(extractedTypesMap).map((entry) : string => {
		...
		const renderedExtends = typeInfo.extends.map(typeDefinition => typeDefinition.value).join(' & ');
		return (typeInfo.exported ? 'export ' : '' ) + `type ${name} = ${renderedExtends ? renderedExtends + ' & ' : ''}${renderedSubType}`;
	}).join('\n\n');
...
const typescriptTypeForOptionsConfig = (config : OptionsConfig | OptionsConfigMap, optional = false, description : string = undefined ) : TypeDefinition => {
	...
	if (config.typeInfo) {
		...
		if (typeInfoIsImported(typeInfo)) {
			return {
				type: 'import',
				value: typeInfo.typeName,
				import:	typeInfo.import,
				optional,
				description
			};
		}
		...
		const extendsInfo : ImportTypeDefinition[] = (typeInfo.extends || []).map(subTypeInfo => {
			return {
				type: 'import',
				value: subTypeInfo.typeName,
				import: subTypeInfo.import
			};
		});
		return {
			type: 'extracted',
			...
			exported: typeInfo.exported ? true : false,
			extends: extendsInfo,
			optional,
			description
		};
