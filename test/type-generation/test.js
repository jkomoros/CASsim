...
export type ${DEMO_SIMULATOR_NAME_CAMEL_CASED}SimOptions = {
	...
	it('basic result extracted sub-type duplicate', async () => {
		const input = {
			foo: {
				example: 3,
				optional: true,
		...
		try {
			simulatorTypeFileContents(DEMO_SIMULATOR_NAME, input, DEMO_SIMULATOR_FILENAME);
		} catch (err) {
			//We expect this to error currently
