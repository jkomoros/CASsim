...
class MultiSelect extends LitElement {
	...
	render() {
		...
		return html`<details>
			...
			${Object.entries(defaultedOptions).map((entry, index, entries) => html`<input id=${entry[0]} type='checkbox' .disabled=${this.disabled || (index == 0 && noOptionsSelected)} .checked=${index == 0 ? noOptionsSelected : (values[entry[0]] || entries.length == 1)} .value=${entry[0]} @change=${this._handleCheckboxChanged}></input><label for=${entry[0]}>${index == 0 && entries.length > 1 ? html`<em>${entry[1]}</em>` : entry[1]}</label>`)}
		</details>
		`;
